...
async function runFullImport() {
  console.log("🚀 TÜM ÜRÜNLERİN YÜKLEMESİ BAŞLATILIYOR (6.901 ÜRÜN)...");

  fs.mkdirSync(UPLOAD_DIR, { recursive: true });

  const client = new ConvexHttpClient(PROD_CONVEX_URL);

  // 1. Reset Database & Categories/Brands
  console.log("\n🧹 1. Adım: Canlı veritabanı temizleniyor ve kategoriler/markalar sıfırlanıyor...");
  const initRes = await client.mutation(api.importData.resetAndInitAll, {});
  const categoryMap = initRes.categoryMap;
  console.log(`✅ Eski ürünler temizlendi (${initRes.deletedProducts} adet). Kategoriler hazır.`);

  // 2. Read All Files From ZIP
  console.log("\n📦 2. Adım: data.zip okunuyor ve ürünler şablonlanıyor...");
  const psScript = `
  ...
  $list | Out-File -FilePath 'temp_full_files.txt' -Encoding utf8
  $zip.Dispose()
  `;

  fs.writeFileSync("temp_read_full.ps1", psScript);
  execSync("powershell -ExecutionPolicy Bypass -File temp_read_full.ps1");
  try { fs.unlinkSync("temp_read_full.ps1"); } catch (e) { }

  const rawLines = fs
    .readFileSync("temp_full_files.txt", "utf8")
    .replace(/^\uFEFF/, "")
    .split("\n")
    .map((l) => l.trim())
    .filter(Boolean);

  try { fs.unlinkSync("temp_full_files.txt"); } catch (e) { }
  console.log(`✅ Toplam ${rawLines.length} görsel bulundu.`);

  // Group images by clean product code
  ...
  console.log(`📦 Toplam ${allProducts.length} adet benzersiz ürün oluşturuldu.`);
...
Beyindeposu güvencesiyle kaliteli çıkma otomotiv elektronik yedek parçaları.`;

      const metaTitle = `${item.code} ${brand} ${catSpec.name} Orijinal Çıkma | Beyindeposu`;
      const metaDescription = `${brand} araçlar için ${item.code} numaralı ${catSpec.name}. Test edilmiş orijinal çıkma parça ve otomotiv elektronik ürünleri Beyindeposu güvencesiyle.`;
      const metaKeywords = `${item.code}, ${brand} ${catSpec.name}, ${brand} çıkma parça, ${catSpec.unitType}, oto beyin, otomotiv elektronik, Beyindeposu`;
      const tags = [item.code, brand, catSpec.name, catSpec.unitType, "Orijinal Çıkma", "Otomotiv Elektronik", "Beyindeposu"];
    ...
    process.stdout.write(
      `\rİlerleme: %${percent} (${currentCount}/${allProducts.length} ürün aktarıldı - Toplam: ${totalInserted})`
    );
  }

  console.log(`\n\n🎉 BİTTİ! Toplam ${totalInserted} ürün Convex Canlı Veritabanına eksiksiz aktarıldı!`);
}
